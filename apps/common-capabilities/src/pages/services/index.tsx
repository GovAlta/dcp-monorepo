...
export default function HomePage(): JSX.Element {
    ...
    const [checkedFilters, setCheckedFilters] = useState(() => {
        const savedCheckboxState = localStorage.getItem(
    ...
    useEffect(() => {
        if (!isLoading && data) {
            const services = data.services.filter(
                (s) =>
                    includeDecommissioned || s.status !== Status.Decommissioned,
            );
            setApps(services);
        ...
        content = (
            ...
            >
                ...
                <span className="last-updated">
                    Showing {recommendedServices.length + otherServices.length}{' '}
                    of {apps.length} results{' '}
                </span>
                <GoASpacer vSpacing="s" />
                <h2>Recommended services listing</h2>
                Recommended services are standard components built for the
                product teams to reuse. We highly recommend leveraging these
                standard services with the "Recommended" tag to streamline your
                development process, maximize efficiency, and optimize costs.
                <GoASpacer vSpacing="xl" />
                ...
                Other services include services built to serve specific use
                cases and might not be suitable to be used by the product teams.
                We still encourage you to the reach out to the service providers
                to collaborate or share knowledge and best practices if you are
