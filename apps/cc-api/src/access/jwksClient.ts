...
export async function getJwksClient(
    ...
    if (issuerCached) {
        logger.info(
            `Found cached JWKS client option for iss '${iss}'.'`,
            LOG_CONTEXT,
        );
        return jwksClient;
    }
    ...
    const oidcUrl = new URL(
        `/auth/realms/${environment.REALM}/.well-known/openid-configuration`,
        new URL(environment.KEYCLOAK_ROOT_URL),
    );
    const { data } = await axios.get<{ issuer: string; jwks_uri: string }>(
        oidcUrl.href,
    );
    ...
    logger.info(
        `Created and cached JWKS client for iss '${iss}'.'`,
        LOG_CONTEXT,
    );
