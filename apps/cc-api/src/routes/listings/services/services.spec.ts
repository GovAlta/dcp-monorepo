...
describe('services', () => {
  let loggerMock, cacheMock, reqMock, resMock;
  const services = [
    {
      appId: '1',
  ...
  beforeEach(() => {
    ...
    cacheMock = {
      get: jest
        .fn()
        .mockResolvedValue(
          services.reduce((acc, s) => ({ ...acc, [s.appId]: s }), {}),
        ),
  ...
  describe('getServices', () => {
    it('should return 200 and list of services from cache', async () => {
      await getServices(loggerMock, valueServiceUrl, cacheMock)(
        reqMock,
        resMock,
        jest.fn(),
      );
    ...
    it('should return list of services from value service if cache is empty', async () => {
      ...
      expect(axios.get).toHaveBeenNthCalledWith(
        1,
        'http://value.service/common-capabilities/values/published-index?top=1',
        { headers: { Authorization: `Bearer token` } },
      );
      expect(axios.get).toHaveBeenNthCalledWith(
        2,
        'http://value.service/common-capabilities/values/1?top=1',
        { headers: { Authorization: `Bearer token` } },
      );
      expect(axios.get).toHaveBeenNthCalledWith(
        3,
        'http://value.service/common-capabilities/values/2?top=1',
        { headers: { Authorization: `Bearer token` } },
      );
      expect(cacheMock.set).toHaveBeenCalledWith(
        CacheKeys.SERVICES,
        { [services[0].appId]: services[0], [services[1].appId]: services[1] },
  ...
  describe('getService', () => {
    it('should return 200 and service info', async () => {
      ...
      await getService(loggerMock, valueServiceUrl, cacheMock)(
        reqMock,
        resMock,
        jest.fn(),
      );
    ...
    it('should return 404 if service not found', async () => {
      ...
      expect(resMock.send).toHaveBeenCalledWith({
        error: 'service not found with id=does-not-exist',
      });
    });
  });
});
