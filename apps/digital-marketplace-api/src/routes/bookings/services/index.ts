    ...
    return data.map((booking) => {
        ...
        return {
            ...
            isBookable:
                ...
                Number(environment.MAX_BOOKING_PER_SLOT),
        };
    });
}
...
export function getAvailableBookings(
    ...
    return async (req, res) => {
        try {
            ...
            const offsetInHours =
                new Date(timeZoneOffset).getTimezoneOffset() / 60;
            date.setHours(date.getHours() - offsetInHours);
            date.setDate(
                date.getDate() + Number(environment.BOOKINGS_DAYS_AHEAD),
            );
            ...
            const getCurrentBookingsAttendees = await Promise.all(
                getCurrentBookings.data.results.map(async (booking) => {
                    const getBookingAttendees =
                        await axios.get<CalendarEventsData>(
            ...
            const mappedData = {
                ...
                businessDays: mapBusinessDaysData(
                    getBusinessDates.data.results,
                ),
            };
            ...
            mappedData.businessDays.forEach((businessDay) => {
                ...
                if (
                    Object.values(availability).some((isBookable) => isBookable)
                ) {
