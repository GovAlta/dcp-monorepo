...
function getValue(type,orig,ck) {    
    switch (type)
    {
        case "radio":     return ck ? orig: null; 
        case "checkbox": return ck ? orig: null; 
        default: return orig == ''? null : orig;
    }    
}
...
function getFormDataArray() {    
    ...
    inputs.forEach((o) => {
        var currentInput   = {                
            ...
            ,"nameCount": 1
            ,"type": o.type 
            ,"check": ["radio","checkbox"].includes(o.type) ? null : getValidationInfo(o.name)            
        }

        if (currentInput.name == lastInput.name) {  // radio or checkbox            
    ...
    if (checkboxValues.length != 0) { // deal with if last was a checkbox...
        lastInput.value = checkboxValues;
    } else if (lastInput.type == 'checkbox' && lastInput.nameCount == 1 ) {
        lastInput.value = lastInput.checked;     
    }
...
function setErrorMessage(data, ok) {
  data.forEach((input) => {     
    ...
    if (strong == null)
        strong = getStrong(inputElement.parentElement);
...
async function submitForm(formName) {
  try {    
    ...
    isOk = setErrorMessage(dataArray.filter(item => item.isValid), isOk);
    isOk = setErrorMessage(dataArray.filter(item => !item.isValid), isOk);
    ...
    if (isOk) {
      ...
      dataArray.forEach((input) => {
        jsonData[input.name] = input.value == null ? "" :input.value;
      });
      ...
      document.getElementById('responseMessage').textContent = 'Please review your input';
    ...
    document.getElementById('responseMessage').innerHTML = `Error: ${error.message}<br />${error.response.data.errorMessage}`;
